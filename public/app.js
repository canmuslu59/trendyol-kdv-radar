...
async function loadCategories(){ categories=await json('/api/categories'); $('#category').innerHTML='<option value="">Tüm whitelist</option>'+categories.filter(x=>x.enabled).map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join(''); }
...
async function loadProducts(){
 const p=new URLSearchParams(); for(const id of ['q','category','minPrice','maxPrice','minD7','minD30','sort']){const v=$('#'+id).value;if(v)p.set(id,v)}
 ...
 </tr>`).join(''); $('#empty').hidden=rows.length>0;
}
